...
def get_menu_full(id):

    return sql_select_wright('SELECT * FROM menu WHERE kitchen_id = %s;', [id])
...
def active_action(active, id):
    
    return sql_write('UPDATE menu SET active = %s WHERE id = %s;', [active, id])

def delete_food_action(id):

    return sql_write('DELETE FROM menu WHERE id = %s;', [id])
